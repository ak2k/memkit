...
# question the user asked. 14.9% of search-reaching traffic is one of these,
# and the shipped hook injects on 100% of them at the full MAX_HITS — mean
# 3.00 pointers — so a session drains POINTER_BUDGET in ten notifications
# before the user has asked anything. The terms carrying those matches are
# `output` `file` `tool` `code` `claude` `id` `task` `src` `summary` `tmp`
# `exit` `user`: the vocabulary of the scaffolding itself, which is why no
# ranking or floor change can reach this — the matches are real, the reader is
# not. Gated rather than ranked for that reason.
#
...
# it (`<agent-browser> keeps timing out on the kagi login page, can you look?`).
...
# imagination. `<command-message` (68 occurrences), `<bash-input`/`<bash-stdout`
# (22), `<!-- Generated by ce-lite converter` (175) and `[Request interrupted by
# user]` (1683) were all found in the transcript vocabulary while slipping the
# predicate; `<command-message` in particular is the same slash-command family
# as the already-gated `<command-name`, differing only in which tag the harness
# emits first. The exhaustive replay confirms the bar holds — 34645/34645 both
# directions, 0 of 27,902 human-or-mixed prompts touched — but only after the
# stratifier it compares against was given the same five markers, which it had
# not been. Adding a marker here without adding it there does not fail: it
# makes the replay report the addition as a false positive of the hook. The
# tripwire for that is test_the_stratifier_knows_every_marker_the_hook_gates_on.
...
def _is_envelope(stripped: str) -> bool:
    ...
    real messages the two predicates gate the identical 4,735 records, so this
    costs no real envelope; it recovers the paste-then-ask shape that anchoring
    alone suppressed.
    """
...
# Calibrated by transcript replay (7926 tagged injections): floors ~35%,
# top matched terms among floored = "s, fix, see, use, yes, sure" (Zipf
# 4.8-6.4); known-good technical hits unaffected.
...
def _record_matched(
    ...
    200 replayed prompts, scoping to the chunk took the hook from 18 silent
