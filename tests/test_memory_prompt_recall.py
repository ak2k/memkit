...
# Each name judges a path, a descriptor or a repository and ANSWERS: the answer
# is the value, so a call made for its side effect has thrown the judgement
# away and left the caller free to re-derive a different one. `_passes_floor`
# judges relevance rather than a path and is deliberately out.
PATH_GUARDS = (
    ...
    ("_trust_gate", "what an uninitialised install refuses with, or None"),
)


def test_every_path_guard_call_uses_the_value_it_returns() -> None:
    """A guard that judges a path returns the resolution it judged, and a call
    that drops it has judged one path and acted on another. Grep-able as a
    lint, so it is one: no call to any of them may stand alone as a statement,
    and `_` is a spelling of standing alone.

    Two non-vacuity halves, because a lint that silently empties is worse than
    none. Every name must still resolve to a module-level `def`, so a rename
    fails here instead of quietly clearing the set. And the set's in-module
    call sites must still total at least 20: there are 26, one each for
    `_named_dir_flags`, `_cwd_in_root`, `_lex_read_only`, `_lex_marked` and
    `_trust_gate`, two each for `_project_store`, `_store_live_dir`, `_inside`
    and `_lex_root`, three for `_regular_fd`, four for `_repo_root`, six for
    `_store_path`, and none for `path_refusal` — which is in the set anyway,
    being the module's public path judgement, so that the first call site
    someone adds arrives under the rule.
    """
    tree = ast.parse(Path(hook.__file__).read_text(encoding="utf-8"))
    names = {name for name, _reason in PATH_GUARDS}
    defined = {
    ...
    dropped = [
        (n.value.func.id, n.lineno) for n in ast.walk(tree)
    ...
    def discarded(target: ast.expr) -> bool:
    ...
    for n in ast.walk(tree):
        ...
        if all(discarded(t) for t in targets):
            thrown.append((call.func.id, n.lineno))
